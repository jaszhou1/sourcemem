...
```{r}
RTquantiles <- data.frame()


quantiles <- unname(quantile(recogdata$response_RT, probs = c(0.1,0.5,0.9)))

RTquantiles[1,1] <- quantiles[1]
RTquantiles[1,2] <- 0.1
RTquantiles[2,1] <- quantiles[2]
RTquantiles[2,2] <- 0.5
RTquantiles[3,1] <- quantiles[3]
RTquantiles[3,2] <- 0.9
RTquantiles[,3] <- participant



colnames(RTquantiles) <- c("value", "quantile","participant")
...
for (sub in 1:length(data.subjList)){
  ...
  # Run function, return data frame (dataset)
  RTquantiles <- create.subj.string(subjNo, RTquantiles)
  
}

# Create as factor for plotting and also include levels so the order is correct 
RTquantiles$subjString <- factor(RTquantiles$subjString, 
                                 levels = unique(RTquantiles$subjString))

RTquantiles <- RTquantiles[!RTquantiles$participant == 14,]
```
...
continuous <- continuous[continuous$participant == participant,]
threshold <- threshold[threshold$participant == participant,]
hybrid <- hybrid[hybrid$participant == participant,]
```
...
ggplot(data=recogdata,aes(x=recogdata$response_RT, ..density..)) + geom_histogram(bins = 50, color = "black", fill = "white") +
  ...
  # ----Plot RT data quantiles ------


geom_vline(xintercept = RTquantiles$value, group = RTquantiles$quantile, linetype="dotted", size = 1.5)+
  
  geom_text(data = RTquantiles, aes(label = quantile, x = value, y = Inf), hjust = -.1) +
