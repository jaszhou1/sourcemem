...
RTfiltered <- dataset[dataset$response_RT<1.5,]
...
#Find quantiles for RT data
for (sub in 1:20){
  
quantiles <- unname(quantile(recogdata[recogdata$participant == sub,]$response_RT, probs = c(0.1,0.5,0.9)))

this.quant <- data.frame()
this.quant[1,1] <- quantiles[1]
this.quant[1,2] <- 0.1
this.quant[2,1] <- quantiles[2]
this.quant[2,2] <- 0.5
this.quant[3,1] <- quantiles[3]
this.quant[3,2] <- 0.9

this.quant[,3] <- sub


RTquantiles <- rbind(RTquantiles,this.quant)
}
