...
% ========================================================================
% in the Fit_Data structure, {:,1} is Data, {:,2} is unData
Data = conf_data;
...
for i = participants
    ...
    for j = 1:nruns
     ...
     %   fitplot(Data {i}, HY_LL_Preds_Data{i,3});
    end
...
%     fitplot(Data {i}, VP_LL_Preds_Data{i,3});
%     saveas(gcf,filename);
%     
%     filename = ['Thresh_Recog',num2str(i),'_',datestr(now,'dd_mm_yy_HH_MM'),'.png'];
%     fitplot(Data {i}, MX_LL_Preds_Data{i,3});
%     saveas(gcf,filename);
%     
...
% abs_marginal_mat_to_csv(filename, 'Cont', VP_LL_Preds_Data);
% 
% filename = [datestr(now,'yyyy-mm-dd-HH-MM'),'_Thresh','_Marginal','.csv'];
% abs_marginal_mat_to_csv(filename, 'Thresh', MX_LL_Preds_Data);
% 
% filename = [datestr(now,'yyyy-mm-dd-HH-MM'),'_Hybrid','_Marginal','.csv'];
% abs_marginal_mat_to_csv(filename, 'Hybrid', HY_LL_Preds_Data);
% 
% 
% filename = [datestr(now,'yyyy-mm-dd-HH-MM'),'_Cont','_Joint','.csv'];
% abs_mat_to_csv(filename, 'Cont', VP_LL_Preds_Data);
% 
% filename = [datestr(now,'yyyy-mm-dd-HH-MM'),'_Thresh','_Joint','.csv'];
% abs_mat_to_csv(filename, 'Thresh', MX_LL_Preds_Data);
% 
% filename = [datestr(now,'yyyy-mm-dd-HH-MM'),'_Hybrid','_Joint','.csv'];
% abs_mat_to_csv(filename, 'Hybrid', HY_LL_Preds_Data);
