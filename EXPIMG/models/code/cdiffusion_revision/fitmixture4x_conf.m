...
Ub= [ 7*ones(1,4),  1*ones(1,2),  5*ones(1,2),    ones(1,2),   1.0, 0.8, 0]; 
Lb= [0*ones(1,4),  0*ones(1,2),  0.3*ones(1,2),    zeros(1,2),   -0.5, 0, 0];
Pub=[ 6.5*ones(1,4),  0.7*ones(1,2),  4.5*ones(1,2), 1*ones(1,2),  0.8, 0.7, 0]; 
...
%    [tc, gtc, thetac, pthetac, mtc] = vdcircle3cls(Pc, tmax, badix);
%    [td, gtd, thetad, pthetad, mtd] = vdcircle3cls(Pd, tmax, badix);
