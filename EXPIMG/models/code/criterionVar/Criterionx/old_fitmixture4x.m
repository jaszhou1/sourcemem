function [ll,bic,Pred, Gstuff] = old_fitmixture4x(Pvar, Pfix, Sel, Data, nlow, nhigh, trace)
% ========================================================================
...
Lb= [-7.0*ones(1,4),  0.0*ones(1,2),  0.4*ones(1,2),    zeros(1,2),   -0.35,     0];
Pub=[ 6.5*ones(1,4),  3.5*ones(1,2),  4.5*ones(1,2), 0.9*ones(1,2),  0.8,  2.8]; 
Plb=[-6.5*ones(1,4),  0.0*ones(1,2),  0.5*ones(1,2), 0.05*ones(1,2), -0.3, 0];
Pred = cell(1,4);
...
% Ensure etas and a are positive.
% Removed Ter from being considered as we are allowing Ter to be negative.
lowerbounderror = sum(min(P(5:10) - zeros(1,length(P)-6), 0).^2);
if lowerbounderror > 0
    disp('eta/ter/a not positive')
