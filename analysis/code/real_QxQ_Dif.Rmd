...
```{r}
data <- read.csv("QQ_data.csv")
cont <- read.csv("2020-01-17-10-19_Cont_Joint_pooled.csv")
thresh <- read.csv("2020-01-17-10-19_Thresh_Joint_pooled.csv")
hybrid <- read.csv("2020-01-17-10-19_Hybrid_Joint_pooled.csv")
...
# Continuous
cont <- model[model$model_name == 'Cont',]

# Threshold
thresh <- model[model$model_name == 'Thresh',]
...
# Plot continuous model           
  geom_line(data = cont,
            aes(x = cont$theta,
                y = cont$rt,
                group = cont$quantile_idx), size = 0.55, linetype = "dashed") +
  
  # Plot threshold model           
  geom_line(data = thresh,
            aes(x = thresh$theta,
                y = thresh$rt,
                group = thresh$quantile_idx), size = 0.55, linetype = "solid") +
  
  # Plot hybrid model           
  geom_line(data = hybrid,
            aes(x = hybrid$theta,
                y = hybrid$rt,
                group = hybrid$quantile_idx), size = 0.55, linetype = "dotdash") +
...
ggsave('ppoled_QQ', plot = QxQ, device = "eps", path = NULL,
       width = 20, height = 20, 
