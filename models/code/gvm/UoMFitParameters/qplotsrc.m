function qplotsrc(fitfunc, Pvar, Pfix, Sel, Data, Bounds);
% ----------------------------------------------------------------------
% Empirical and RT fitted RT quantiles as a function of theta.
% for source memory experiment.
%       qplotsrc(@fitfunc, Pvar, Pfix, Sel, Data, {Bounds});
% ----------------------------------------------------------------------
    if nargin < 6 
        Bounds = [.3, 2.5]
    end
    ...
    axhandle =setfig2;
    qploti(co, axhandle(1), Data{1}, gtma, ta, thetaa, tmax, minrt, maxrt, 'Long', 0, Bounds);
    qploti(co, axhandle(2), Data{2}, gtma, ta, thetaa, tmax, minrt, maxrt, 'Short', 0, Bounds);
end

function qploti(co, axi, Datai, Gt, T, thetai, tmax, minrt, maxrt, labi, do_xlabel, Bounds)
% =======================================================================
...
% =======================================================================
  ...
  % Empirical RT quantiles in accuracy bins
   ...
   set(gca, 'YLim', [Bounds(1),Bounds(2)]);  % maxrt
   if do_xlabel
       xlabel('Hitting Angle \theta (radians)')
   end
   ylabel('Distribution Quantile Q_i (s)')
