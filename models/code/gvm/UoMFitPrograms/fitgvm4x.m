function [ll,bic,Pred, Gstuff] = fitgvm4x(Pvar, Pfix, Sel, Data, trace)
% ========================================================================
% Circular diffusion with drift variability for Jason's source memory task.
% gvm with criterion variability
%
%    [ll,bic,Pred, Gstuff] = fitgvm4(Pvar, Pfix, Sel, Data)
%     P=[nunorm1, nunorm2, kappa1, kappa2, rho, a, Ter, st, sa]
%          1        2        3        4     5   6    7   8   9
...
% ----------------------------------------------------------------
%   nunorm1, nunorm2, kappa1, kappa2, rho, a,    ter, st     sa
% ----------------------------------------------------------------
 
Ub= [ 7.0*ones(1,2), 11.0*ones(1,2),  1.5, 5.0,  1.0, 0.7,  2*a]; 
Lb= [   0*ones(1,2),  0.0*ones(1,2), 0.02, 0.5, -0.35,  0     0];
Pub=[ 6.5*ones(1,2), 10.5*ones(1,2),  1.3, 4.5,  0.8, 0.65, 2*a - .02]; 
Plb=[0.01*ones(1,2),  0.1*ones(1,2), 0.03, 0.7,  -0.40, 0.01,  0];
Pred = cell(1,4);
if any(P - Ub > 0) | any(Lb - P > 0)
   ...
   return
    ...
    if sa < epsilon
        Pa=[nunorm1, kappa1, phi rho, sigma, a];
        % Parameters for long
        Pb=[nunorm2, kappa2, phi, rho, sigma, a];
        % Parameters for short
        [ta, gta, thetaa, pthetaa, mta] = gvm300(Pa, tmax,badix);
        [tb, gtb, thetab, pthetab, mtb] = gvm300(Pb, tmax, badix);
    else
        ...
        mtb = zeros(1, nw+1);
        for i = 1:n_sa_step
            Pai=[nunorm1, kappa1, phi rho, sigma, A(i)];
            Pbi=[nunorm2, kappa2, phi, rho, sigma, A(i)];
            [ta, gtai, thetaa, pthetaai, mtai] = gvm300(Pai, tmax, badix);
            [tb, gtbi, thetab, pthetabi, mtbi] = gvm300(Pbi, tmax, badix);         
            gta = gta + gtai;
            gtb = gtb + gtbi;
