function data = source_memory_experiment(seed, subj_number, ...
    temp_directory, date_string, word_list, directory_separator)
...
design = generate_source_memory_design(seed, par, word_list);
...
for i = 1:par.num_blocks
    ...
    for j = 1:num_encode_trials
        ...
        if strcmp(encode_cond,'SEQUENTIAL') == 1
    ...
    this_block_maths_data = maths_distractor(scr, 30); %Change back to 30s
    ...
    this_block_error = vertcat(this_block_test_data.reproduction_error);
    this_block_error = abs(this_block_error);
    test_data = [test_data; this_block_test_data];
    
    % Display accuracy feedback for block

    source_memory_feedback(scr,par,this_block_error,i)
