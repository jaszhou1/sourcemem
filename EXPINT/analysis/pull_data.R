# PULL_DATA.R
# Get each participants' data and concatenate, output as a .csv
...
# Function to get all participant data and put it together
n_sessions <- 10
get_all_data <- function(){
  data <- data.frame(matrix(nrow = 0, ncol = 57))
  for(i in 1:length(completed.users)){
    this.participant.data <- data.frame(matrix(nrow = 0, ncol = 57))
    for(j in 1:n_sessions){
      this.data <- get_session(i, j)
      this.data <- append_intrusions(this.data)
      this.participant.data <- rbind(this.participant.data, this.data)
    }
    data <- rbind(data, this.participant.data)
  }
  return(data)
}