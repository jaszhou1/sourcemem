...
# This model assumes spatial, temporal, orthographic, and semantic
# similarity are combined multiplicatively to determine a non-target
# probability of intruding.

orthosem_model <- function(params, data){
  ...
  # Spatiotemporal
  # Item
  iota <- params[5] # Similarity decay of orthographic component
  upsilon <- params[6] # Similarity decay of semantic component
  psi <- params[7] # Weight of semantic vs orthographic in item component
  ...
  intrusion_weights <- (orthographic_similarity^(1-psi)) * (semantic_similarity^psi)
...
simulate_pure_orthosem_model <- function(participant, data, params){
