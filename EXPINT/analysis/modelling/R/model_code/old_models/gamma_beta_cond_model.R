...
gamma_beta_cond_model <- function(params, data){
  ...
  # GUESS
  beta1 <- params[3] # Proportion of guesses (weight vs. intrusion+memory)
  beta2 <- params[4] # Orth cond.
  beta3 <- params[5] # Sem cond.
  # INTRUSIONS
  gamma1 <- params[6] # Overall scaling of intrusions, this is not directly interpretable
  gamma2 <- params[7] # Orth cond,
  gamma3 <- params[8] # Sem cond.
  # Spatiotemporal
  tau <- params[9] # Temporal asymmetry (tau >0.5 means forwards are more similar)
  lambda_b <- params[10] # Similarity decay of backwards temporal lag
  lambda_f <- params[11] # Similarity decay of forwards temporal lag
  zeta <- params[12] # Similarity decay of spatial similarity
  rho <- params[13] # Weight of spatial vs temporal in spatiotemporal component
  chi1 <- params[14] # Weight of item vs spatiotemporal similarity component LOW
  chi2 <- params[15] # Weight of item HIGH
  # Item
  iota1 <- params[16] # Similarity decay of orthographic component LOW (unrelated/semantic)
  iota2 <- params[17] # Decay for orthography HIGH 
  upsilon1 <- params[18] # Similarity decay of semantic component LOW
  upsilon2 <- params[19] # Decay for semantic HIGH
  psi1 <- params[20] # Weight of semantic vs orthographic in item component LOW
  psi2 <- params[21] # Weight of semantic vs orthographic in item component HIGH
...
simulate_gamma_beta_cond_model <- function(participant, data, params){
