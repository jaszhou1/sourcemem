...
orthographic.similarities <- ggplot(data = orth.data) +
  geom_bar(aes(x = value, fill = condition)) +
  facet_wrap(~condition, nrow = 1) +
  scale_x_continuous(name = 'Orthographic (Damerau-Levenshtein) Distance', 
                   breaks = 1:6, 
...
sem.similarities <- ggplot(data = sem.data) +
  geom_histogram(aes(x = value, fill = condition), bins = 30) +
  facet_wrap(~condition, nrow = 1) +
  scale_x_continuous(name = 'Semantic (word2vec Cosine) Similarity',
...
plot <- ggarrange(orthographic.similarities, NULL, sem.similarities, ncol = 1, heights = c(1, 0.1, 1))
ggsave('list_comparison.png', plot = last_plot(), width = 40, height = 20, units = "cm")