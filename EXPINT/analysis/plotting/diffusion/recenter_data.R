...
recenter.model <- function(data){
  ...
  for (i in 1:nrow(data)){
    ...
    this_response_error <- data[i, 1]
    this_target_angle <- data[i, 4]
    this_intrusion_offsets <- data[i, 6:12]
    ...
    for(j in 1:n_intrusions){
      ...
      recentered_errors[idx,3] <- this_intrusion_offsets[[j]]
      recentered_errors[idx,4] <- angle_diff(this_response_error, this_intrusion_offsets[[j]])
      recentered_errors[idx,5] <- temporal[[j]]
